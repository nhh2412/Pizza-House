            ...
            for (var j = 0; j < values.length; j++) {
                ...
                for (var k = j; k >= -1; k--) {
                    ...
                    if (
                        ...
                        responsive[values[j]]['stagePadding'] =
                            k < 0 ? 0 : parseInt(c.attr('data' + aliaces[k] + 'stage-padding'), 10)
                    }
                    if (
                            ...
                            if (isNoviBuilder ? false : c.attr('data-loop') !== 'false') {
                                ...
                                if (tempFix > 0) {
                                    numberingObject
                                        ...
                                        .text(tempFix > e.item.count ? tempFix % e.item.count : tempFix)
                                } else {
                    ...
                    ;(function (bootstrapTabsItem) {
                        bootstrapTabsItem.on('shown.bs.tab', function (event) {
                            var prevTriggerable = bootstrapTabsItem.find(
                                    '[data-view-trigger="' + event.relatedTarget.getAttribute('href') + '"]',
                                ),
                                triggerable = bootstrapTabsItem.find(
                                    '[data-view-trigger="' + event.target.getAttribute('href') + '"]',
                                )
                    ...
                    onSlideChangeStart: (function (s) {
                        return function (swiper) {
                            ...
                            if (swiper.slides[activeSlideIndex - 1].getAttribute('data-slide-title')) {
                                $(swiper.container).find('.swiper-button-next .title')[0].innerHTML =
                                    swiper.slides[activeSlideIndex + 1].getAttribute('data-slide-title')
                                $(swiper.container).find('.swiper-button-prev .title')[0].innerHTML =
                                    swiper.slides[activeSlideIndex - 1].getAttribute('data-slide-title')
                            }

                            if (swiper.slides[activeSlideIndex - 1].getAttribute('data-slide-subtitle')) {
                                $(swiper.container).find('.swiper-button-prev .subtitle')[0].innerHTML =
                                    swiper.slides[activeSlideIndex - 1].getAttribute('data-slide-subtitle')
                                $(swiper.container).find('.swiper-button-next .subtitle')[0].innerHTML =
                                    swiper.slides[activeSlideIndex + 1].getAttribute('data-slide-subtitle')
                            }
                            //Replace btn img
